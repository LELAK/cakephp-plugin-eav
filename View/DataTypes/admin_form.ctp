...
if ($this->request->params['action'] == 'admin_edit') {
    $this->Html
            ->addCrumb(__d("eav", "Tipo de Dado"), array('admin' => true, 'plugin' => 'eav', 'controller' => 'data_types', 'action' => 'index'))
            ->addCrumb($this->request->data['EavDataType']['name'], '#');
}
...
$this->Form->create('EavDataType', array('url' => '/' . $this->request->url,));
$inputDefaults = $this->Form->inputDefaults();
$inputClass = isset($inputDefaults['class']) ? $inputDefaults['class'] : null;

$this->append('tab-heading');
echo $this->Croogo->adminTab(__d("eav", "Tipo de Dado"), '#data-type-basic');
echo $this->Croogo->adminTabs();
$this->end();

$this->append('tab-content');

echo $this->Html->tabStart('data-type-basic');
